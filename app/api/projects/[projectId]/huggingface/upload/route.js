      ...
      if (!repoExists) {
        try {
          await createRepo({
            ...
            private: isPrivate,
            license: 'mit',
            description: project.description || 'Dataset created with MED-Dataset'
          });
...
async function uploadFile(token, datasetName, filePath, destFileName) {
  try {
    ...
    await uploadFiles({
      ...
      commitDescription: `Files uploaded using MED-Dataset`
    });
...
## Description
${projectDescription || 'This dataset was created using the MED-Dataset tool.'}

## Format
This dataset is in ${formatType} format.

## Creation Method
This dataset was created using the MED-Dataset tool.

> MED-Dataset is a professional medical AI dataset creation and management platform for medical large model training.
